...
class HomeParsing(object):
	...
	def ExtractData(self,indirizzo,nome_doc,funzioni, referer):
		...
		if req.status_code == 403 and "idealista" in indirizzo:
			...
			w.title("Sblocco richiesto")
			w.geometry("350x150")
			ttk.Label(w,text="Idealista ci ha negato l'accesso. Visita la home di idealista digitando l'indirizzo direttamente dalla barra dell'url del tuo browser preferito e clicca su \"non sono un robot\".",padding = [0,10,0,10]).pack()
