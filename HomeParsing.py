	...
	def ExtractAnnunci(self,indirizzo,funzione,next,referer):
		...
		while next(pagina,indirizzo) != False:
			...
			if req.status_code == 403 and "idealista" in indirizzo_n:
				...
				w.geometry("350x150")
				label2 = ttk.Label(w,wraplength=300,text="Idealista ci ha negato l'accesso. Visita la home di idealista digitando l'indirizzo direttamente dalla barra dell'url del tuo browser preferito e clicca su \"non sono un robot\".",padding = [0,10,0,10])
